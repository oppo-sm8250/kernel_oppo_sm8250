...
#include "wireless_ic/oplus_p922x.h"	//for WPC
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_VOOCCHG_ING:
			...
			if (!val->intval && chip->wireless_support) {
				val->intval = p922x_wpc_get_fast_charging();  
			}
#else
			if (!val->intval && chip->wireless_support) {
...
void oplus_charger_detect_check(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->check_chrdet_status()) {
		...
		if (chip->wireless_support && p922x_wpc_get_fw_updating() == true) {
			return;
		}
...
#ifndef WPC_NEW_INTERFACE
        if ((chip->charger_exist || p922x_wireless_charge_start()) 
...
#else
        } else if ((chip->charger_exist || oplus_wpc_get_status()) && chip->batt_exist && (CHARGING_STATUS_FAIL != chip->charging_state) && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)) {
#endif        
		chip->sleep_tm_sec = 0;
		chip->prop_status = POWER_SUPPLY_STATUS_CHARGING;
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	...
	if (chip->vbatt_num != 1 && chip->prop_status != POWER_SUPPLY_STATUS_CHARGING) {
...
static void oplus_chg_other_thing(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_exist) {
		if (chgr_dbg_total_time != 0) {
			...
			if (!chip->total_time) {
				get_rtc_time(&start_chg_time);
				chip->total_time += OPLUS_CHG_UPDATE_INTERVAL_SEC;
			} else {
				get_rtc_time(&cur_chg_time);
...
int oplus_chg_get_chg_ichging(void)
{
        if (!g_charger_chip) {
                return 250;
        } else {
