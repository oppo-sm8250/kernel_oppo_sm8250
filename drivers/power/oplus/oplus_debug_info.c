...
int __attribute__((weak)) get_pmk_ldoi_err_cnt(void)
{
	return 0;
}
int __attribute__((weak)) get_pmk_ldoj_err_cnt(void)
{
	return 0;
}

int __attribute__((weak)) oplus_get_haptic_error_status(void)
{
	return 0;
}
int __attribute__((weak)) oplus_get_pmk_error_status(void)
{
	return 0;
}
...
static void oplus_chg_print_debug_info(struct oplus_chg_chip *chip)
{
		...
		oplus_chg_debug_notify_flag_is_set(OPLUS_NOTIFY_CHARGER_INFO))
	{
		...
		/*add for haptic*/
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				"$$haptic_error_status@@%d", oplus_get_haptic_error_status());

		/*add for pm8010 or pm8008*/
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				"$$ldoi@@%d", get_pmk_ldoi_err_cnt());
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				"$$ldoj@@%d", get_pmk_ldoj_err_cnt());
		ret += snprintf(&oplus_chg_debug_msg[ret], OPLUS_CHG_DEBUG_MSG_LEN - ret,
				"$$get_pmk_status@@%d", oplus_get_pmk_error_status());
