...
int qg_get_battery_temp(struct qpnp_qg *chip, int *temp)
{
	...
	if (rc < 0) {
		pr_err("Failed reading BAT_TEMP over ADC rc=%d, voltage=%d\n", rc, voltage);
		return rc;
	}

	pre_voltage = voltage;
	pre_voltage = div64_s64(pre_voltage, 1000);
	rc = oplus_asic_vadc_map_voltage_temp(adcmap_batt_therm_asic,
				 ARRAY_SIZE(adcmap_batt_therm_asic),
				 pre_voltage, &pre_result);

	if(g_oplus_qg_ibta < -6000000)
		g_oplus_qg_ibta = -6000000;
	if(g_oplus_qg_ibta > 6000000)
		g_oplus_qg_ibta = 6000000;
	voltage = voltage + g_oplus_qg_ibta * 6;
