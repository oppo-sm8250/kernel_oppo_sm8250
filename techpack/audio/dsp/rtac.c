...
int get_voice_mute_state(u32 mode, u32 opcode, u32 paramID)
{
    ...
    struct apr_hdr voice_params;
    //Wei.Hua@MULTIMEDIA.AUDIODRIVER.PLATFORM, 2020/06/12, avoid to call before voice session is ready
    ...
    */
    ...
    pr_info("%s: mute_detect finally result*****************%x\n",__func__, result);
    mutex_unlock(&rtac_voice_apr_mutex);
    return result;
done:
    return bytes_returned;
...
#endif /* OPLUS_FEATURE_MUTE_DETECT */
