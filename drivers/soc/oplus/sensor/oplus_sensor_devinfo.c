...
static void normalization_value(struct sensor_hw* hw, struct device_node *ch_node)
{
    int rc = 0;
    int value = 0;
    hw->feature.feature[6] = 1057;
    rc = of_property_read_u32(ch_node, "normalization_value", &value);
    if (!rc) {
        hw->feature.feature[6] = value;
        SENSOR_DEVINFO_DEBUG("normalization_value : %d\n", hw->feature.feature[6]);
    } else {
        pr_info("use default normalization parameters!\n");
    }
}
...
static void parse_light_sensor_dts(struct sensor_hw* hw, struct device_node *ch_node)
{
    ...
    normalization_value(hw, ch_node);
}
...
static void parse_color_temperature_sensor_dts(struct sensor_hw* hw, struct device_node *ch_node)
{
    int rc = 0;
    int value = 0;
...
static int oplus_als_cali_data_init()
{
    ...
    gdata->proc_oplus_als =  proc_mkdir("oppoAls", NULL);
