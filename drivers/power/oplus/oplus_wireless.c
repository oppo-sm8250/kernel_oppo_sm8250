...
int oplus_wpc_get_status(void)
{
    if(!g_wpc_chip) {
        return OPLUS_WPC_NO_CHARGER_MODE;
    }

    if (g_wpc_chip->charge_online == true) {
        if (g_wpc_chip->fastchg_ing == true)
            return OPLUS_WPC_FASTCHG_MODE;
        else
            return OPLUS_WPC_NORMAL_MODE;
    }

    return OPLUS_WPC_NO_CHARGER_MODE;
}

int oplus_wpc_set_wpc_enable(bool enable)
{
    if(!g_wpc_chip) {
    ...
    oplus_wpc_set_wpc_en_val(enable);
    return 0;
}

int oplus_wpc_set_wired_otg_enable(bool enable)
{
    if(!g_wpc_chip) {
		chg_err("g_wpc_chip is not ready\n");
        return 0;
    ...
    oplus_wpc_set_wrx_en_value(enable);
    oplus_wpc_set_otg_en_val(enable);
    oplus_wpc_set_booster_en_val(enable);
    return 0;
}

int oplus_wpc_set_wpc_otg_enable(bool enable)
{
    if(!g_wpc_chip) {
		chg_err("g_wpc_chip is not ready\n");
        return 0;
    ...
    if (enable == true) {
        ...
        oplus_wpc_set_wrx_otg_value(enable);
        oplus_chg_set_charger_otg_enable(!!enable);
        msleep(1000);
...
int oplus_wpc_set_wired_chg_enable(bool enable)
{
    if(!g_wpc_chip) {
		chg_err("g_wpc_chip is not ready\n");
        return 0;
    ...
    oplus_wpc_set_wpc_enable(enable);
}

int oplus_wpc_set_switch_default_mode(void)
{
    oplus_wpc_set_wpc_enable(true);
    oplus_wpc_set_wired_otg_enable(false);
    oplus_wpc_set_wpc_otg_enable(false);

    return 0;
}

int oplus_wpc_switch_mode(int work_mode, bool enable)
{
    if(!g_wpc_chip) {
		chg_err("g_wpc_chip is not ready\n");
        return 0;
	...
	switch (work_mode){
		case OPLUS_WPC_SWITCH_DEFAULT_MODE :
            oplus_wpc_set_switch_default_mode();
			break;
		case OPLUS_WPC_SWITCH_WIRED_CHG_MODE :
            oplus_wpc_set_wired_chg_enable(enable);
            if (enable) {
                oplus_wpc_set_wired_otg_enable(false);
                g_wpc_chip->wpc_ops->set_dischg_enable(g_wpc_chip , enable);
                msleep(100);
			...
			break;
		case OPLUS_WPC_SWITCH_WPC_CHG_MODE :
			break;
		case OPLUS_WPC_SWITCH_WIRED_OTG_MODE :
            oplus_wpc_set_wired_otg_enable(enable);
			break;
		case OPLUS_WPC_SWITCH_WPC_OTG_MODE :
            oplus_wpc_set_wpc_otg_enable(enable);
			break;
		case OPLUS_WPC_SWITCH_OTHER_MODE :
            oplus_wpc_set_switch_default_mode();
			break;
	}
}

static int oplus_wpc_en_gpio_init(struct oplus_wpc_chip *chip)
{
...
static void oplus_wpc_set_wpc_en_val(int value)  // 0  active, 1 inactive
{
    struct oplus_wpc_chip *chip = g_wpc_chip;

	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: wpc not ready!\n", __func__);
		return;
	}
...
int oplus_wpc_get_wpc_en_val(void)
{
    struct oplus_wpc_chip *chip = g_wpc_chip;
...
static irqreturn_t oplus_wpc_connect_int_handler(int irq, void *dev_id)
{
	struct oplus_wpc_chip *chip = dev_id;

	if (!chip) {
		chg_err(" oplus_wpc_chip is NULL\n");
	} else {
...
static void oplus_wpc_con_irq_init(struct oplus_wpc_chip *chip)
{
...
static void oplus_wpc_con_eint_register(struct oplus_wpc_chip *chip)
{
	int ret = 0;
	...
	ret = devm_request_threaded_irq(chip->dev, chip->wpc_gpios.wpc_con_irq,
			NULL, oplus_wpc_connect_int_handler, 
...
static int oplus_wpc_con_gpio_init(struct oplus_wpc_chip *chip)
{
	chip->wpc_gpios.pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chip->wpc_gpios.pinctrl)) {
...
static int oplus_wpc_vbat_en_gpio_init(struct oplus_wpc_chip *chip)
{
	if (!chip) {
...
void oplus_wpc_set_vbat_en_val(struct oplus_wpc_chip *chip, int value)
{    
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_wpc_chip not ready!\n", __func__);
		return;
...
int oplus_wpc_get_vbat_en_val(struct oplus_wpc_chip *chip)
{
	if (chip->wpc_gpios.vbat_en_gpio <= 0) {
		chg_err("vbat_en_gpio not exist, return\n");
		return -1;
...
static int oplus_wpc_booster_en_gpio_init(struct oplus_wpc_chip *chip)
{
...
int oplus_wpc_get_booster_en_val(void)
{
    struct oplus_wpc_chip *chip = g_wpc_chip;
...
static int oplus_wpc_otg_en_gpio_init(struct oplus_wpc_chip *chip)
{
    printk(KERN_ERR "[OPLUS_CHG][%s]: tongfeng test start!\n", __func__);
...
int oplus_wpc_get_otg_en_val(struct oplus_wpc_chip *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: oplus_wpc_chip not ready!\n", __func__);
		return -1;
...
static int oplus_wpc_wrx_en_gpio_init(struct oplus_wpc_chip *chip)
{
    printk(KERN_ERR "[OPLUS_CHG][%s]: tongfeng test start!\n", __func__);
...
static int oplus_wpc_wrx_otg_gpio_init(struct oplus_wpc_chip *chip)
{
    printk(KERN_ERR "[OPLUS_CHG][%s]: tongfeng test start!\n", __func__);
...
void oplus_wpc_set_wrx_otg_value(int value)
{
    struct oplus_wpc_chip *chip = g_wpc_chip;
...
static int oplus_wpc_wired_conn_gpio_init(struct oplus_wpc_chip *chip)
{
    printk(KERN_ERR "[OPLUS_CHG][%s]: tongfeng test start!\n", __func__);
...
static void oplus_wpc_wired_conn_irq_init(struct oplus_wpc_chip *chip)
{
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: wpc not ready!\n", __func__);
		return;
    ...
    printk(KERN_ERR "[OPLUS_CHG][%s]: chip->wpc_gpios.wired_conn_irq[%d]!\n", __func__, chip->wpc_gpios.wired_conn_irq);
}

static void oplus_wpc_wired_conn_irq_register(struct oplus_wpc_chip *chip)
{
	int ret = 0;
	...
	ret = devm_request_threaded_irq(chip->dev, chip->wpc_gpios.wired_conn_irq,
			NULL, oplus_wpc_wired_conn_change_handler, IRQF_TRIGGER_FALLING
...
static bool oplus_wpc_wired_conn_check_is_gpio(struct oplus_wpc_chip *chip)
{
    if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: wpc not ready!\n", __func__);
		return false;
...
static int oplus_wpc_parse_param_dt(struct oplus_wpc_chip *chip)
{
    int rc = 0;
...
int oplus_wpc_gpio_dt_init(struct oplus_wpc_chip *chip)
{
    int rc = 0;
    struct device_node *node = chip->dev->of_node;
    ...
    if (chip->wpc_gpios.wpc_en_gpio <= 0) {
        ...
        if (gpio_is_valid(chip->wpc_gpios.wpc_en_gpio)) {
            ...
            if (rc) {
                ...
                rc = oplus_wpc_en_gpio_init(chip);
                if (rc)
	...
	if (chip->wpc_gpios.wpc_con_gpio < 0) {
		...
		if (gpio_is_valid(chip->wpc_gpios.wpc_con_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_wpc_con_gpio_init(chip);
				if (rc)
					chg_err("unable to init wpc_con_gpio:%d\n", chip->wpc_gpios.wpc_con_gpio);
				else {
					oplus_wpc_con_irq_init(chip);
					oplus_wpc_con_eint_register(chip);
				}
			}
		}
	...
	if (chip->wpc_gpios.wired_conn_gpio < 0) {
		...
		if (gpio_is_valid(chip->wpc_gpios.wired_conn_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_wpc_wired_conn_gpio_init(chip);
				if (rc)
					chg_err("unable to init wired_conn_gpio:%d\n", chip->wpc_gpios.wired_conn_gpio);
				else {
					oplus_wpc_wired_conn_irq_init(chip);
					oplus_wpc_wired_conn_irq_register(chip);
				}
			}
		}
	...
	if (chip->wpc_gpios.vbat_en_gpio < 0) {
		...
		if (gpio_is_valid(chip->wpc_gpios.vbat_en_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_wpc_vbat_en_gpio_init(chip);
				if (rc) {
	...
	if (chip->wpc_gpios.booster_en_gpio < 0) {
		...
		if (gpio_is_valid(chip->wpc_gpios.booster_en_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_wpc_booster_en_gpio_init(chip);
				if (rc) {
	...
	if (chip->wpc_gpios.otg_en_gpio < 0) {
		...
		if (gpio_is_valid(chip->wpc_gpios.otg_en_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_wpc_otg_en_gpio_init(chip);
				if (rc) {
	...
	if (chip->wpc_gpios.wrx_en_gpio < 0) {
		...
		if (gpio_is_valid(chip->wpc_gpios.wrx_en_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_wpc_wrx_en_gpio_init(chip);
				if (rc) {
	...
	if (chip->wpc_gpios.wrx_otg_gpio < 0) {
		...
		if (gpio_is_valid(chip->wpc_gpios.wrx_otg_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_wpc_wrx_otg_gpio_init(chip);
				if (rc) {
...
static int oplus_wpc_get_con_gpio_val(struct oplus_wpc_chip *chip)
{
	if (chip->wpc_gpios.wpc_con_gpio <= 0) {
...
static int oplus_wpc_init_connected_task(struct oplus_wpc_chip *chip)
{
...
static int oplus_wpc_deinit_after_disconnected(struct oplus_wpc_chip *chip)
{
	if (chip->charge_online) {
		chip->charge_online = false;
		oplus_wpc_reset_variables(chip);
		chargepump_disable();
		oplus_wpc_set_vbat_en_val(chip, 0);
	}

	return 0;
}

static bool oplus_wpc_is_firmware_updating(void)
{
	if (!g_wpc_chip) {
		return 0;
	}
...
static void oplus_wpc_connect_int_func(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_wpc_chip *chip = container_of(dwork, struct oplus_wpc_chip, wpc_connect_int_work);
    
    if (oplus_wpc_is_firmware_updating() == true) {
        return;
    }

	if (oplus_wpc_get_con_gpio_val(chip) == 1) {
		chg_err("<~WPC~> wpc dock has connected!>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
		oplus_wpc_init_connected_task(chip);		
		schedule_delayed_work(&chip->wpc_task_work, WPC_TASK_INTERVAL);
	} else {
		chg_err("<~WPC~> wpc dock has disconnected!< < < < < < < < < < < < <\n");
		oplus_wpc_deinit_after_disconnected(chip);
	}
}

void oplus_wpc_init(struct oplus_wpc_chip *chip)
{
	INIT_DELAYED_WORK(&chip->wpc_connect_int_work, oplus_wpc_connect_int_func);
	INIT_DELAYED_WORK(&chip->wpc_task_work, oplus_wpc_task_work_process);
}


static void oplus_wpc_reset_variables(struct oplus_wpc_chip *chip)
{
	int i;
...
static int oplus_wpc_RXTX_message_process(struct oplus_wpc_chip *chip)
{
	send_msg_timer++;
	if (send_msg_timer > 2) {
		send_msg_timer = 0;
...
static void oplus_wpc_get_charging_info(struct oplus_wpc_chip *chip)
{
    chip->wpc_ops->get_vrect_iout(chip);
    return;
}

static int oplus_wpc_temperature_status_process(struct oplus_wpc_chip *chip)
{
    int tbat_temp = oplus_chg_get_chg_temperature();
...
static bool is_allow_wpc_fast_chg(struct oplus_wpc_chip *chip)
{
    int batt_temp = 0;
    int batt_volt = 0;
    int batt_soc = 0;
    ...
    batt_volt = oplus_chg_get_batt_volt();
    batt_soc = oplus_chg_get_soc();
...
static int oplus_wpc_charge_status_process(struct oplus_wpc_chip *chip)
{
	...
	case WPC_CHG_STATUS_DEFAULT:
		...
		if (chip->temperature_status == TEMPERATURE_STATUS_NORMAL) {
			if ((g_oplus_chip->batt_volt_min > P922X_PRE2CC_CHG_THD_LO)
				&& (g_oplus_chip->batt_volt_max < P922X_CC2CV_CHG_THD_HI)
				&& (g_oplus_chip->soc <= 90)) {
				if (chip->adapter_type == ADAPTER_TYPE_FASTCHAGE_VOOC) {                    
	...
	case WPC_CHG_STATUS_INCREASE_VOLTAGE:
		...
		if (p922x_get_CEP_flag(chip) == 0) {
			chg_err("<~WPC~> .g_oplus_chip->batt_volt * 4 + 500: %d\n", g_oplus_chip->batt_volt * 4 + 500);
	...
	case WPC_CHG_STATUS_FAST_CHARGING_FROM_CHGPUMP:
		...
		if (g_oplus_chip->batt_volt >= 4500 
            || ((-1 * g_oplus_chip->icharging) < 1000 && g_oplus_chip->batt_volt >= 4370 && chip->p922x_chg_status.wpc_4370_chg == false)) {
			batt_vol_4450_cnt++;
			if (batt_vol_4450_cnt < 5) {
				;//break;
			}

			chg_err("<~WPC~> The max cell vol >= 4500mV.g_oplus_chip->icharging[%d]\n", g_oplus_chip->icharging);
			...
			break;
		        ...
		        || (g_oplus_chip->batt_volt >= 4370 || chip->p922x_chg_status.wpc_4370_chg)) {
			batt_vol_4450_cnt = 0;
			...
			} else if ((g_oplus_chip->temperature >= 400) || (chip->p922x_chg_status.iout < 600)){
				chg_err("<~WPC~> Turn to MP2650 control\n");
				p922x_ready_for_mp2650_charge(chip);
				chip->p922x_chg_status.charge_status = WPC_CHG_STATUS_FAST_CHARGING;
			}
		...
		if(adjust_current_delay > 0) {
			...
			if (g_oplus_chip->temperature >= 400) {
				...
				if ((chip->p922x_chg_status.charge_level == CHARGE_LEVEL_39_TO_41_DEGREE)
					|| (chip->p922x_chg_status.charge_level == CHARGE_LEVEL_ABOVE_41_DEGREE)){
					if(g_oplus_chip->temperature < 360) {
...
//			}
		...
		if (g_oplus_chip->batt_volt >= 4450) {
			batt_vol_4450_cnt++;
			if (batt_vol_4450_cnt < 5) {
				break;
			}
...
static void oplus_wpc_task_work_process(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oplus_wpc_chip *chip = container_of(dwork, struct oplus_wpc_chip, wpc_task_work);
	chg_err("tongfeng test charge_online[%d]\n", chip->charge_online);

	if (chip->charge_online) {
        oplus_wpc_get_charging_info(chip);
		//p922x_get_vrect_iout(chip);

		oplus_wpc_RXTX_message_process(chip);
		oplus_wpc_temperature_status_process(chip);
		oplus_wpc_charge_status_process(chip);   ///////////////// at here
