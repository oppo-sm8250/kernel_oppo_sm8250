		...
		(struct oplus_vooc_chip *chip, int vbat_temp_cur)
{
	...
	switch (chip->fastchg_batt_temp_status) {
		case BAT_TEMP_NATURAL:
			if (vbat_temp_cur > chip->vooc_strategy1_batt_high_temp0) {
				...
				if (chip->vooc_batt_over_low_temp != -EINVAL) {
					ret = 2;
					vooc_xlog_printk(CHG_LOG_CRTI, "for cell balancing, the ret:%d\r\n", ret);
				}
			}
			break;
		case BAT_TEMP_COOL:
			if (chip->vooc_batt_over_low_temp != -EINVAL
				...
				if (chip->vooc_strategy_change_count >= COUNTS) {
					...
					chip->fastchg_batt_temp_status = BAT_TEMP_OVER_LOW_EXIT;
					ret = chip->vooc_over_high_or_low_current;
				}
			} else if (chip->vooc_batt_over_low_temp != -EINVAL
					&& vbat_temp_cur > chip->vooc_batt_over_high_temp) {
				chip->vooc_strategy_change_count++;
				if (chip->vooc_strategy_change_count >= COUNTS) {
					chip->vooc_strategy_change_count = 0;
					chip->fastchg_batt_temp_status = BAT_TEMP_OVER_HIGH_EXIT;
					ret = chip->vooc_over_high_or_low_current;
				}
			} else if (chip->vooc_batt_over_low_temp != -EINVAL
					&& vbat_temp_cur < chip->vooc_cool_temp) {
				chip->vooc_strategy_change_count++;
				if (chip->vooc_strategy_change_count >= COUNTS) {
					chip->vooc_strategy_change_count = 0;
					chip->fastchg_batt_temp_status = BAT_TEMP_COOL;
		...
		case BAT_TEMP_LOW2:
			...
			break;
		case BAT_TEMP_OVER_HIGH_EXIT:
			chip->vooc_strategy_change_count = 0;
			chip->fastchg_batt_temp_status = BAT_TEMP_OVER_HIGH_EXIT;
			ret = chip->vooc_over_high_or_low_current;
			break;
		case BAT_TEMP_OVER_LOW_EXIT:
			chip->vooc_strategy_change_count = 0;
			chip->fastchg_batt_temp_status = BAT_TEMP_OVER_LOW_EXIT;
			ret = chip->vooc_over_high_or_low_current;
			break;
